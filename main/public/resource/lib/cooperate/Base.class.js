...
class Base {
    ...
    ajax(url, {
        ...
        fetch(url, req)
            .then(response => {
                //返回状态码
                if (typeof status === 'function') status(response.status);
                //直接返回内容
    ...
    on(selecter, ...params) {
        ...
        if (!this._eventMap.get(target).has(evt)) {
            ...
            target.addEventListener(evt, e => {
                ...
                //冒泡
                for (; domLen + 1; --domLen) {
                    this._onFunction(e.path[domLen], target, e.type, e.target);
                }
            }, useCapture)
        }
    ...
    _onFunction(node, target, event, realTarget) {
        ...
        keyArr.forEach(key =>
            (eventTarget.get(key) && eventTarget.get(key).has(event))
            && eventTarget.get(key).get(event).forEach(v => v(node, realTarget)))
    }
...
!window.coo ? window.coo = new Base() : console.warn('coo 全局变量已被占用！');